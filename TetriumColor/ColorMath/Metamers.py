...
def GetMetamerBuckets(points: npt.ArrayLike, axis: int = 2, prec: float = 0.005, exponent: int = 8) -> List:
    sorted_points = []
    buckets = SortBuckets(BucketPoints(
        points, axis=axis, prec=prec, exponent=exponent), axis=axis)
...
def _getRefinedHypercube(metamer_led_weights: npt.ArrayLike, previous_step: float) -> npt.ArrayLike:
    ...
    """
    outer_range = [[metamer_led_weights[i] - previous_step,
                    metamer_led_weights[i] + previous_step]for i in range(4)]
...
def _getTopKMetamers(M_WeightsInCone: npt.ArrayLike, hypercube: npt.ArrayLike, metameric_axis: int, K: int, prec: float = 0.005, exponent: int = 8) -> npt.ArrayLike:
    ...
    """
    all_lms_intensities = (
        M_WeightsInCone@hypercube.T).T  # multiply all possible led combinations with the intensities
    buckets = GetMetamerBuckets(
        all_lms_intensities, axis=metameric_axis, prec=prec, exponent=exponent)
    ...
    # return in display space weight coordinates
    return metamers.reshape(K, 2, 4)
...
def _refineMetamers(metamers: npt.ArrayLike, M_weightToCone: npt.ArrayLike, metameric_axis: int = 2, hypercube_sample: float = 0.01) -> npt.ArrayLike:
    ...
    for met in tqdm(metamers):
        refined = _refineMetamer(met[0], met[1], M_weightToCone,
                                 metameric_axis=metameric_axis, hypercube_sample=hypercube_sample)
...
def GetKMetamers(transform: ColorSpaceTransform, K: int, hypercubeSample: float = 0.05) -> List[PlateColor]:
    ...
    # takes no time at 0.05.
    hypercube = GetSampledHyperCube(hypercubeSample, 4)
    metamers_first_pass = _getTopKMetamers(
