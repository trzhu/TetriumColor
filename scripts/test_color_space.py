...
def test_color_space():
    ...
    print("\nTest 2: Initialize ColorSpace with sRGB display")
    try:
        cs_sRGB = ColorSpace(observer, cst_display_type='sRGB')
    ...
    cs_4d = ColorSpace(observer, cst_display_type='led', display_primaries=gaussian_smooth_primaries)
  ...
  # Create a test point in VSH space (value, saturation, hue)
    ...
    if cs_4d.dim == 4:
        vsh_point = np.array([[1.0, 0.3, 0.0, 0.0]])  # For 4D
    ...
    # Convert to RGB_OCV
    rgb_ocv_point = cs_4d.convert(vsh_point, ColorSpaceType.VSH, ColorSpaceType.DISP_6P)
    ...
    in_gamut_orig = cs_4d.is_in_gamut(vsh_point, ColorSpaceType.VSH)
    in_gamut_high = cs_4d.is_in_gamut(out_of_gamut_point, ColorSpaceType.VSH)
    ...
    print(f"Remapped point in gamut: {cs_4d.is_in_gamut(remapped_point, ColorSpaceType.VSH)}")
